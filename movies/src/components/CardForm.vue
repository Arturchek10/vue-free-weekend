  ...
  <div class="container">
    <div class="card" v-for="item in movies" :key="item.id">
      ...
      <div class="text-block">
        ...
        <div class="genre">
          <button id="btn-genre">{{ item.genres[0] }}</button>
        </div>
        ...
        <div class="rating">
          ...
          <StarIcon
            ...
            :key="star"
            :index="index"
            :isClick="item.isClick && index < item.rating"
            @changeStarToggle="toggleStar(item, index)"
          />
        </div>
      </div>
    </div>
...
.text-block {
  ...
  display: flex;
  flex-direction: column; /*Внутренние элементы располагаем вертикально*/
  padding: 20px;
}
